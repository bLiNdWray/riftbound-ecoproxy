...
(() => {
  ...
  // Exposed counts (used by both build() and UI)
  window.cardCounts = {};
  ...
  function makeRune(c) {
    const cols = (c.colors||'').split(/[;,]\s*/).filter(Boolean);
    const img  = cols[0] ? cols[0] : 'Body';
    return build(c.variantNumber, `
      <div class="rune-title">${c.name}</div>
      <div class="rune-image">
        <img src="images/${img}.png" alt="${c.name}">
      </div>`);
  }
  ...
  // ── Expose & Wrap addCard/removeCard ───────────────────────────────
  ...
  document.getElementById('btn-full-proxy').addEventListener('click', ()=>{
    fullProxy = !fullProxy;
  ...
  // ── UI: Mutation Observer & Initial Load ──────────────────────────
  new MutationObserver(() => {
    updateCount();
    Object.keys(window.cardCounts).forEach(vn => refreshBadge(vn));
